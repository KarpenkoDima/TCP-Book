...
### 4.7 RFC и черновики IETF

В отличие от большинства протокольных механизмов книги, congestion control — зона, где практика опережает стандартизацию:

* **RFC 5681** — TCP Congestion Control (актуальная база: Slow Start, Congestion Avoidance, Fast Retransmit/Recovery — то, что мы называли "Reno").
* **RFC 8312** — CUBIC for Fast Long-Distance Networks (формальное описание того, что разобрано в 4.2).
* **RFC 3168** — ECN (основа для DCTCP, 4.4).
* **RFC 8257** — DCTCP.
* **RFC 8289 / RFC 8290** — CoDel и FQ-CoDel (основа AQM, 4.5).
* **BBR — НЕ RFC.** До сих пор существует только как IETF draft (`draft-cardwell-iccrg-bbr-congestion-control`) в рабочей группе ICCRG. Важно понимать: то, что стоит по умолчанию на серверах Google, YouTube и части CDN — не прошло формальную стандартизацию IETF. Это показатель того, что в congestion control де-факто стандарт определяется имплементацией в ядре Linux, а не RFC-процессом — обратная ситуация по сравнению с TCP core (Модуль 3), где RFC 9293 первичен.

---

### 4.8 Что посмотреть в Wireshark

Помимо `ss -ti`, разобранного в Case Study, полезна визуальная проверка:

* **Statistics → TCP Stream Graphs → Window Scaling** — на CUBIC-соединении виден классический "зубец" (рост → потеря → просадка до 0.7×), на BBR — плато без пилы, потому что BBR не реагирует на единичные потери.
* **io graph по bytes-in-flight** — самый наглядный способ показать разницу BDP-targeting (BBR) от blind-filling (CUBIC) без чтения `ss` вывода построчно.

---

### 4.9 Как это реализовано в Windows

Ключевой факт, который часто удивляет тех, кто настроен на Linux-терминологию: **Windows по умолчанию не использует ни CUBIC, ни BBR.**

* Начиная с Windows Vista, дефолтный алгоритм — **Compound TCP (CTCP)**, собственная разработка Microsoft: комбинирует потери (как Reno) с задержкой (delay-based компонент, идейно ближе к Vegas/BBR, но не идентичен). Именно поэтому прямое сравнение "throughput на Windows vs Linux" на одном и том же линке часто даёт разные числа даже без искусственных настроек.
* Windows Server 2019+ добавил поддержку CUBIC как альтернативы, но **BBR в tcpip.sys нет** — ни как встроенного алгоритма, ни как экспериментального (в отличие от `net.ipv4.tcp_congestion_control=bbr` в Linux, который включается одной командой).

**Диагностика и переключение:**

```powershell
# Текущий congestion provider (аналог sysctl tcp_congestion_control)
Get-NetTCPSetting | Select-Object SettingName, CongestionProvider

# Переключение на CUBIC (доступно, BBR — нет)
Set-NetTCPSetting -SettingName InternetCustom -CongestionProvider CUBIC

# Аналог ss -ti по глубине детализации в Windows недоступен через штатные
# командлеты — delivery_rate/pacing_rate/app_limited нет.
# Для эквивалентной глубины нужен ETW-провайдер Microsoft-Windows-TCPIP
# и разбор в Windows Performance Analyzer — ss -ti на Windows не имеет
# однокомандного аналога, это отдельная задача под WPA (см. Модуль 8, 8.9).
```

Практический вывод для .NET-разработчика: если сервис крутится на Windows Server и жалуется на throughput на длинном RTT — сначала проверь, что вообще нет BBR как опции, прежде чем воспроизводить рецепты из этой главы один в один.

---

### 4.10 Production-проблемы

Case Study (Амстердам→Нью-Йорк) закрывает сценарий "потери на длинном RTT". Ниже — два других класса инцидентов, не пересекающихся с ним.

**1. BBR vs CUBIC unfairness на общем bottleneck**
Когда BBR-поток и CUBIC-поток конкурируют за один и тот же узкий буфер, BBR систематически отбирает больше полосы: BBR не снижает cwnd на единичные потери так резко, как CUBIC, и в результате CUBIC-поток проигрывает конкуренцию за очередь. Это задокументированная проблема самого Google (см. обсуждения в IETF ICCRG) и одна из причин, почему массовый rollout BBR v1 в публичном интернете вызывал жалобы у операторов, не использующих BBR сами. BBR v2 — попытка это исправить через более консервативный response на потери, но полностью проблема не снята.

**2. AQM в DOCSIS-сетях (кабельные модемы)**
Долгое время кабельные провайдеры массово ставили модемы с буферами на сотни миллисекунд без AQM (тот самый bufferbloat из 4.1, но не гипотетический, а в проде у миллионов домохозяйств). Проект **CableLabs Active Queue Management** (часть DOCSIS 3.1) заставил производителей включить AQM (обычно PIE, вариант CoDel) по умолчанию — до этого домашний ping под нагрузкой в 200-1000мс был нормой, а не аномалией. Показательный пример того, что AQM — это не только серверная настройка `tc qdisc`, но и то, что должно быть на CPE-устройстве, которое вы не контролируете.
...
### 4.11 Практический проект: TcpCongestionSimulator

Единственный новый файл поверх проекта из 3.7.2 — `Core/CongestionController.cs`.
Он меняет поведение отправителя во всей остальной системе: usable window
теперь `min(cwnd, rwnd) - bytesInFlight` вместо просто `rwnd`, как было в
3.4.4 и 3.7.2.

**Три режима (RFC 5681 §3.1–3.2):**
...
3 duplicate ACK (fast retransmit) -> ssthresh = max(flight/2, 2·MSS)
                                      cwnd = ssthresh + 3·MSS
RTO истёк (тяжелее)                -> ssthresh = max(flight/2, 2·MSS)
                                      cwnd = MSS  (заново slow start)
```

Разница между ветками намеренная: RTO значит "мы вообще не видели ответа от
пира", fast retransmit значит "пир жив, у нас просто gap" — поэтому RTO
откатывает `cwnd` гораздо консервативнее (полный слоу-старт, а не просто
уменьшение вдвое).

**Initial window (RFC 5681 §3.1)** реализован буквально:
...
При MSS=4 (тестовый прогон) это 16 байт — то есть initial cwnd реально
ограничивает первый залп четырьмя сегментами, даже если окно получателя
намного больше. Это то же самое ограничение, что и в теоретической части
4.2 ("сколько занимает разгон CUBIC/BBR до BDP"), только здесь оно видно на
уровне одного объекта, а не асимптотики.

**Тот же сценарий, что в 3.7.2, но с cwnd** — видно, как congestion control
не просто наблюдает, а навязывает темп отправки: пятый и шестой сегменты в
3.7.2 уходили сразу же (`t=0`), здесь — только к `t≈1.002`, потому что до
этого момента `cwnd` был физически исчерпан.

**Что осталось нереализованным** (честно, а не молча): `OnDuplicateAckDuringRecovery`
(RFC 5681 §3.2, шаг "inflate") ни разу не сработал в тестовом прогоне —
recovery длился ровно от 3-го dup ACK до следующего же ACK, оказавшегося
финальным; NewReno partial ACK (RFC 6582) не реализован — весь backlog
доставился одним каскадом; pacing отсутствует — `TrySendMore` выпускает весь
доступный кусок окна одним циклом, без равномерного распределения во
времени. Это открытые направления для читателя, а не upcoming-фичи.
