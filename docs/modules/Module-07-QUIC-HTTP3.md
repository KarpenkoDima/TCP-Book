...
### 7.12 RFC — ядро стандарта QUIC

QUIC — редкий случай в книге, когда стандарт моложе большинства практикующих инженеров (финализирован в мае 2021), и весь модуль держится на этих документах:

* **RFC 9000** — QUIC: A UDP-Based Multiplexed and Secure Transport (основной транспортный протокол — то, что разобрано в 7.2–7.7).
* **RFC 9001** — Using TLS to Secure QUIC (крипто-рукопожатие, 7.3).
* **RFC 9002** — QUIC Loss Detection and Congestion Control (user-space congestion control, 7.6 — обрати внимание: здесь QUIC ОБЯЗАН реализовать то, что в TCP делает ядро, отсюда и упомянутый в 7.10 CPU overhead).
* **RFC 9114** — HTTP/3 (7.8).
* **RFC 9204** — QPACK: Field Compression for HTTP/3 (7.8, отличие от HPACK).
* **RFC 9443** — qlog (уже процитирован, но это дополнительный формат логирования, не транспорт — стоит явно отделить от четвёрки выше).

Полезная деталь для читателя: в отличие от TCP (RFC 9293, единый документ), QUIC изначально спроектирован модульно — транспорт, крипто и congestion control разнесены по разным RFC. Это отражает то, что сама IETF QUIC WG сознательно делала протокол расширяемым с первого дня, а не по историческим причинам, как у TCP.

---

### 7.13 Что посмотреть в Linux (и почему тут почти нечего смотреть)

Честно: аналога `ss -ti` для QUIC **не существует**, и это не пробел в тулинге, а прямое следствие архитектуры — QUIC-соединение не имеет представления в ядре, всё состояние живёт в user-space библиотеке (msquic, quiche, quic-go).

Что реально доступно на уровне ОС:

```bash
# Видно только сам UDP-сокет — без ретрансмиссий, cwnd, RTT
ss -u -a

# Через eBPF можно трейсить входящие/исходящие UDP-датаграммы,
# но интерпретация QUIC-фреймов внутри — уже задача userspace-библиотеки
bpftrace -e 'kprobe:udp_sendmsg { printf("pid %d sending UDP\n", pid); }'
```

Вся содержательная диагностика (retransmissions, RTT, cwnd, packet loss) переносится из ядра в приложение — отсюда критическая важность `qlog` (7.10): это не «ещё один формат логов», а **единственный** способ увидеть внутреннее состояние QUIC-соединения, потому что стандартных kernel-инструментов для этого не существует в принципе.

---

### 7.14 Как это реализовано в Windows

QUIC на Windows — часть **msquic** (открытая библиотека Microsoft, та же, что использует и `System.Net.Quic` в .NET, и сам Windows для SMB over QUIC).

* **Требования по версии ОС** — полноценная поддержка QUIC с TLS 1.3 в режиме ядра (Schannel) требует **Windows Server 2022 / Windows 11** и новее. На более старых версиях `System.Net.Quic` в .NET либо не работает, либо требует OpenSSL-based fallback — это стоит проверить перед деплоем сервиса с QUIC на Windows Server 2019.
* **msquic поддерживает два TLS-провайдера**: Schannel (нативный, только Windows) и OpenSSL (кроссплатформенный). На Linux/.NET msquic всегда собран с OpenSSL — то есть поведение TLS-стека реально отличается между ОС, не только по производительности, но и по набору доступных cipher suite.
* **Диагностика** — ETW-провайдер `Microsoft-Quic` даёт трейсинг на уровне соединений, стримов и потерь пакетов, аналогично тому, как `qlog` работает в quiche/quic-go. Смотрится через WPA (см. Модуль 8, 8.9) так же, как обычный TCP ETW-трейс.

```powershell
# Проверка версии Windows на поддержку kernel-mode QUIC (Schannel)
Get-ComputerInfo | Select-Object WindowsProductName, OsVersion

# ETW-трейсинг QUIC через msquic provider
netsh trace start provider=Microsoft-Quic capture=yes tracefile=c:\quic.etl
```

---

### 7.15 Production-проблемы

* **UDP GSO как единственная точка обхода отсутствия hardware offload.** Без `UDP_SEGMENT` (GSO для UDP, доступен в Linux с 4.18+) throughput QUIC-сервера на высоких нагрузках упирается в CPU задолго до упора в NIC — Cloudflare и Fastly публично описывали, что именно отсутствие offload было причиной перехода части инфраструктуры на io_uring/AF_XDP для QUIC-терминации, а не просто масштабирование по ядрам.
* **Anti-amplification limit (RFC 9000 §8.1) как источник обрывов в асимметричных сетях.** Пока клиент не подтвердит адрес (address validation), сервер обязан ограничить объём ответа троекратным размером полученных от клиента данных — это защита от amplification-атак, но на практике при потере первого клиентского пакета в системах со спутниковым/geo-стационарным линком это может приводить к неожиданным задержкам старта, которые выглядят как "баг", а на деле — требование стандарта безопасности.
* **Ossification со стороны middlebox всё ещё встречается.** Часть корпоративных firewall и DPI-систем блокирует или троттлит UDP/443, заставляя полагаться на Happy Eyeballs v2 (7.11, уже описан) как обязательный, а не опциональный fallback — то есть в production нельзя просто "включить QUIC", всегда нужен TCP-путь наготове.
